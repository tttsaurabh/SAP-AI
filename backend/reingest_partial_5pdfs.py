"""
reingest_partial_5pdfs.py
=========================
Partial re-ingest: only first 5 PDFs for quick validation.
Same as reingest_with_gemini_embeddings.py but limits to 5 docs.

Usage:
    cd backend
    .\\venv\\Scripts\\python.exe reingest_partial_5pdfs.py
"""
...
def drop_and_recreate_vector_table():
    ...
    and recreate it fresh with 768 dims for Gemini text-embedding-001.
    """
...
def reset_documents_for_reingest(db: Session, limit: int = None):
    """Mark active/failed documents as 'processing' for re-ingest (limited to N)."""
    query = db.query(Document).filter(Document.status.in_([DocumentStatus.ACTIVE, DocumentStatus.FAILED]))
    if limit:
        docs_to_reset = query.limit(limit).all()
    else:
    ...
    # Also delete old chunk rows
    db.query(Chunk).delete(synchronize_session=False)
    db.commit()
    logger.info(f"Reset {count} documents to 'processing' (limited to {limit}) and cleared chunk rows.")
    return count
...
def main():
    logger.info("=" * 70)
    logger.info("RE-INGESTION SCRIPT (PARTIAL - 5 PDFs ONLY)")
    ...
    # Step 2: Reset only first 5 documents for re-ingestion
    db = SessionLocal()
    try:
        total_docs = reset_documents_for_reingest(db, limit=5)
        if total_docs == 0:
        ...
        logger.info(f"Re-ingesting {len(docs)} documents (partial)...")
        logger.info("-" * 70)
    ...
    logger.info("=" * 70)
    logger.success(
        f"Partial re-ingestion complete: [OK] {success_count} succeeded  [FAIL] {fail_count} failed"
    )
